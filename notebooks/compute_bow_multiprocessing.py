...
def create_batches(path_name, n_processors):
    ...
    batch_size = round(len(path)/n_processors)
    
    batches = []

    for i in range(0, len(path), batch_size):
        batches.append(path[i:i + batch_size])

    count = 0
    for batch in batches:
        count += len(batch)

    print(f'Created {len(batches)} batches of a total of {count} files')
...
if __name__ == '__main__':
    ...
    print(f'{n_processors} CPUs available')
    ...
    df_final.to_csv(r'chunks_BoW_features2.csv', index = True)
